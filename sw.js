(() => {
    ...
    for (let t = 0; t <= i.length + 1; t++)
        for (let s = 0; s <= i[t]; s++) e.essential.push("/static/lottie/" + (t + 1) + "/images/img_" + s + ".png");
    for (let t = 1; t <= 8; t++) e.essential.push("/static/svg/" + t + ".svg");
    ["in", "ln", "tw", "md"].forEach(t => {
        e.essential.push("/static/footer/" + t + ".svg")
    });
    var n = "sw-v1.1",
        a = e.essential;
    self.addEventListener("install", t => {
        t.waitUntil(caches.open(n).then(s => s.addAll(a)))
    });
...
function update(source) {
  ...
  nodeEnter
    ...
    .attr("id",function(d){
      return ("id"+d.id) || (d.id = ("id"+(++i)));
    })
    .attr("x", 0)
    .attr("y", -10)
    .attr("width", function(d) {
      return d.parent ? 50 : 100;
    })
    .attr("height", 20);
  ...
  nodeEnter
    ...
    .attr("text-rendering", function(d) {
